		...
		function strCount(str) {
			if (str != '') {
				...
				var arr = str.split("\n")
				str = arr.join('')
				var str_del_space = str.replace(/\s*/g, "")
				var space_num = str.length - str_del_space.length
				var strCnt = "段落数（不含空行）：" + arr.length
				strCnt += "   字数（不含空格）：" + str_del_space.length
				strCnt += "   空格数：" + space_num
				$("#str_count").text(strCnt);
			}else{
				$("#str_count").text("字符为空！");	
			}
		};
		...
		$("#join_para").click(function(){
		var str = $("#txtarea").val();
		checkInputStr(str);	
		str = str.replace(/^\s*|\s*$/g, "");
		var add_str = prompt("请输入连接符",",");
			if (add_str != null){
				str = str.replace(/\n+/g, add_str)		
				strCount(str);
			}
		...
		checkInputStr(str);	
		var arr = str.split("\n");
		var add_str = prompt("请输入内容","helloworld");
		for (var i = 0; i < arr.length; i++) {
			if (arr[i] != '') {
				arr[i] = add_str + arr[i];
			}
		...
		checkInputStr(str);	
		var arr = str.split("\n");
		var locationNum = prompt("请输入位置，如1表示在第一个字符后面插入内容",1);
		if(isRealNum(locationNum)==2){
		var add_str = prompt("请输入内容","helloworld");		
		for (var i = 0; i < arr.length; i++) {
			if (arr[i] != '') {
				arr[i] = arr[i].substr(0,locationNum) + add_str + arr[i].substr(locationNum);
			}
		}		
		str = arr.join('\n');
		strCount(str);
		}else if(isRealNum(locationNum)==3){alert("需输入大于0的整数");}
		});	
		...
		$("#del_before").click(function(){
		var str = $("#txtarea").val();
		checkInputStr(str);	
		var arr = str.split("\n");
		var add_str = prompt("请输入内容","（");		
		for (var i = 0; i < arr.length; i++) {
			if (arr[i] != '') {
				arr[i] = arr[i].substr(arr[i].indexOf(add_str));
			}
		}		
		str = arr.join('\n');
		strCount(str);
		...
		for (var i = 0; i < arr.length; i++) {
			if (arr[i] != '') {
				arr[i] = arr[i].substr(0,arr[i].indexOf(add_str)+1);
			}
		}		
		str = arr.join('\n');
		strCount(str);
		...
		$("#del_dup_items").click(function(){
		var str = $("#txtarea").val();
		checkInputStr(str);	
		var add_str = prompt("请输入内容","-");
		var dic = new Array();		
		str = str.replace(/\n/g, add_str)
		while (str != str.replace(add_str + add_str, add_str)) {
		str = str.replace(add_str + add_str, add_str)
		}
		if (str.indexOf(add_str) == 0) {str = str.substring(1)}
		if (str.substring(str.length) == add_str) {
		str = str.substring(0, str.length - 1)
		}
		var arr = str.split(add_str)
		for (var i = 0; i < arr.length; i++) {
			if (!(arr[i] in dic) && arr[i] != '') {
				dic[arr[i]] = 0
			}
		}
		var str2 = '';
		...
		var arr2=new Array();
		var n = 0;
		var add_str = prompt("请输入删除字符数",1);		
		if(isRealNum(add_str)==2){		
			for (var j = 0; j < arr.length; j++) {
			// 当该段落字数大于输入数字时，才执行
			if (arr[j].length > add_str) {
				arr2[n] = arr[j].substring(add_str);
				n++;
			}}
			str = arr2.join("\n");
			strCount(str);
		}else if(isRealNum(add_str)==3){alert("需输入大于0的整数");}
		});	

		$("#del_tail").click(function(){
		var str = $("#txtarea").val();
		checkInputStr(str);	
		var arr = str.split("\n");
		var arr2=new Array();
		...
		if(isRealNum(add_str)==2){		
			...
			// 当该段落字数大于输入数字时，才执行
			if (arr[j].length > add_str) {
				arr2[n] = arr[j].substring(0,arr[j].length-add_str);
				n++;
			}}
			str = arr2.join("\n");
			strCount(str);
		...
		$("#del_mid").click(function(){
		var str = $("#txtarea").val();
		checkInputStr(str);	
		var arr = str.split("\n");
		var arr2=new Array();
		var n = 0;
		var add_str1 = prompt("开始删除字符的位置数A",1);	
		var add_str2 = prompt("最后删除字符的位置B",2);
		//判断是否为整数
		if(isRealNum(add_str1)==2 && isRealNum(add_str2)==2){
			if(add_str1>add_str2){
				alert("最后字符的位置数B需大于或等于开始删除字符的位置数A")
					}else{
			for (var j = 0; j < arr.length; j++) {
            // 当该段落字数大于输入数字时，才执行
			var len=arr[j].length;
			var tmp_arr=[];
            if (len >= add_str1 && len >= add_str2) {
              tmp_arr.push(arr[j].substring(0, add_str1 - 1))
              tmp_arr.push(arr[j].substring(add_str2, len))
              arr2[n] = tmp_arr.join('')
              n++              
            }
			}
		...
		}else if(isRealNum(add_str1)==3 || isRealNum(add_str2)==3){alert("需输入大于0的整数");}
		});	


		
		$("#pick_head").click(function(){
		var str = $("#txtarea").val();
		checkInputStr(str);	
		var arr = str.split("\n");
		var arr2=[];
		var n = 0;
		var add_str = prompt("提取前N个字符，请输入字符数",1);		
		if(isRealNum(add_str)==2){		
		   for (var j = 0; j < arr.length; j++) {
            arr2[j] = arr[j].substring(0, add_str)
          }
			str = arr2.join("\n");
			strCount(str);
		}else if(isRealNum(add_str)==3){alert("需输入大于0的整数");}
		...
		$("#pick_tail").click(function(){
		var str = $("#txtarea").val();
		checkInputStr(str);	
		var arr = str.split("\n");
		var arr2=[];
		var n = 0;
		var add_str = prompt("提取后N个字符，请输入字符数",1);		
		if(isRealNum(add_str)==2){		
			for (var j = 0; j < arr.length; j++) {
			// 当该段落字数大于输入数字时，才执行
			var len = arr[j].length
			if (len > add_str) {
				arr2[j] = arr[j].substring(len - add_str, len)
			}else{
				arr2[j] = arr[j]
			}
			}
		...
		$("#pick_mid").click(function(){
		var str = $("#txtarea").val();
		checkInputStr(str);	
		var arr = str.split("\n");
		var arr2=new Array();
		var n = 0;
		var add_str1 = prompt("请输入开始提取位置数A",1);	
		var add_str2 = prompt("请输入最后提取位置数B",2);			
		if(isRealNum(add_str1)==2 && isRealNum(add_str2)==2){
			if(add_str1 > add_str2){
				alert("位置A需大于位置B")
			}else{
				for (var j = 0; j < arr.length; j++) {
					var len = arr[j].length
					if (add_str1 <= len) {
					arr2[n] = arr[j].substring(add_str1-1, add_str2)
					n++
					}
				}
				str = arr2.join("\n");
		...
		$("#pick_para").click(function(){
		var str = $("#txtarea").val();
		checkInputStr(str);	
		var arr = str.split("\n");
		var arr2=[];
		var j = 0;
		var add_str = prompt("请输入筛选关键词","文本");		
		if(isRealNum(add_str) !=1 ){
			if (str.indexOf(add_str) == -1) {
          alert('找不到字符');
        }else{
			for (var i = 0; i < arr.length; i++) {
				if (arr[i].indexOf(add_str) != -1) {
					arr2[j] = arr[i];
					j++;
				}
			}
		...
		$("#drop_para").click(function(){
		var str = $("#txtarea").val();
		checkInputStr(str);	
		var arr = str.split("\n");
		var arr2=[];
		var j = 0;
		var add_str = prompt("请输入剔除关键词","小程序");		
		if(isRealNum(add_str) !=1 ){
			if (str.indexOf(add_str) == -1) {
          alert('找不到字符');
        }else{
		...
		for (var i = 0; i < str.length; i++){
			if(cmp.indexOf(str.charAt(i))!=-1){
				cstr+=smp.charAt(cmp.indexOf(str.charAt(i)));				
			}else{
				cstr+=str.charAt(i);
			}
		}
		...
		var cstr='';
		for (var i = 0; i < str.length; i++){
			if(smp.indexOf(str.charAt(i))!=-1){
				cstr+=cmp.charAt(smp.indexOf(str.charAt(i)));				
			}else{
				cstr+=str.charAt(i);
			}
		}
		...
		$("#str2lines").click(function(){
		var str = $("#txtarea").val();
		checkInputStr(str);	
		var add_str = prompt("请输入分隔符","-");
		if (add_str != null){
			var arr = ["$", "*", "(", ")", "+", ".", "[", "?", "\\", " ^ ", "{", "|"]
			for (var j = 0; j < arr.length; j++) {
			if (add_str == arr[j]) { add_str = '\\' + add_str }
			}
			var re = new RegExp(add_str + "+", "ig")
			str = str.replace(re, '\n')		
			strCount(str);
		}
		});	
		
		$("#newline_in_head").click(function(){
		var str = $("#txtarea").val();
		checkInputStr(str);	
		var add_str = prompt("请输入分隔符","（");
		if (add_str != null){
			...
			var re = new RegExp(add_str + "+", "ig");
			mystr = '\n' + add_str;
			str = str.replace(re, mystr);		
			strCount(str);
		}
		});	
		
		$("#newline_in_tail").click(function(){
		var str = $("#txtarea").val();
		checkInputStr(str);	
		var add_str = prompt("请输入分隔符","）");
		if (add_str != null){
